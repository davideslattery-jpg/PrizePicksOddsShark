"""Match DFS (PrizePicks / Underdog) legs to book fair odds and rank by edge."""
...
class RankedEdge:
    ...
    adjusted: bool
    sport: str = ""
    platform: str = "prizepicks"  # prizepicks | underdog

    def to_dict(self) -> dict[str, Any]:
        return asdict(self)


def parse_dfs_arg(dfs: str | Sequence[str] | None) -> list[str]:
    """Parse --dfs value: prizepicks | underdog | both | comma list."""
    if dfs is None:
        return ["prizepicks", "underdog"]
    if isinstance(dfs, (list, tuple)):
        parts = [str(x).strip().lower() for x in dfs if str(x).strip()]
    else:
        raw = str(dfs).strip().lower()
    ...
    for p in parts:
        if p == "both":
            for k in ("prizepicks", "underdog"):
                if k not in seen:
                    seen.add(k)
                    out.append(k)
            continue
        if p not in DFS_BOOKMAKERS:
            raise ValueError(
                f"Unsupported DFS platform {p!r}. Use prizepicks, underdog, or both."
            )
        if p not in seen:
            seen.add(p)
            out.append(p)
...
def rank_edges(
    ...
    *,
    ...
    min_edge: float = 2.0,
    sport: str | None = None,
    dfs_platforms: Sequence[str] | None = None,
) -> list[RankedEdge]:
    """Build ranked edges from event-odds payloads.

    Ranks each requested DFS bookmaker (prizepicks / underdog) separately
    against the same FanDuel (or Pinnacle) fair probs. One API event payload
    that already includes both DFS books is enough — no second pull.
    """
    platforms = list(dfs_platforms) if dfs_platforms is not None else ["prizepicks"]
    platforms = [p.lower().strip() for p in platforms if p]
    if not platforms:
        platforms = ["prizepicks"]
    for p in platforms:
        if p not in DFS_BOOKMAKERS:
            raise ValueError(f"Unsupported DFS platform: {p}")
    ...
    for event in events:
        ...
        for dfs_key in platforms:
            dfs_legs = extract_props_from_event(
                event, bookmaker_keys=[dfs_key], market_filter=markets
            )
            for pp in dfs_legs:
                ...
                book_opp = (
                    same_line[0] if same_line else nearest_book_leg(opp_cands, book_same.point)
                )
